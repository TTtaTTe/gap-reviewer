...
- 출력 형식: 2개를 구분하여 출력
  1. **표지 메타데이터 JSON** (```json 블록):
     ```json
     {
       ...
       "date": "2026년 3월 24일",
       "author": "작성 부서",
       "classification": "기밀/내부용 등",
       "version": "v2.0"
     }
     ```
     원본 작업물에서 제목, 작성자, 분류 등을 추출하여 채운다.
  2. **수정된 전체 문서 내용** (마크다운 형식)
...
1. Step 4의 수정본 MD 내용을 임시 파일로 저장: `_tmp_content.md`
2. Step 4의 표지 메타데이터 JSON을 임시 파일로 저장: `_tmp_cover.json`
3. 스크립트 실행:
```bash
python "${CLAUDE_SKILL_DIR}/scripts/generate_fix_docx.py" content "{_tmp_content.md 경로}" "{수정본_YYYYMMDD_HHMMSS.docx 경로}" "{_tmp_cover.json 경로}"
```
4. 임시 파일 삭제: `_tmp_content.md`, `_tmp_cover.json`
...
1. Step 5의 수정 이력 JSON을 임시 파일로 저장: `_tmp_modlog.json`
2. 스크립트 실행:
```bash
python "${CLAUDE_SKILL_DIR}/scripts/generate_fix_docx.py" report "{_tmp_modlog.json 경로}" "{수정이력_YYYYMMDD_HHMMSS.docx 경로}"
```
3. 임시 파일 삭제: `_tmp_modlog.json`
